// cavebat_v3.c - the flight that works, plus onboard recording.
//
// This is a copy of cavebat.c (tag v2-full-mission, the version that flew a
// complete mission twice) with exactly ONE capability added: it records a
// sample every second while airborne and hands the recording to the app on
// request over CRTP port 14.
//
// Deliberately NOT changed from the flying version:
//   - the 10Hz loop (v2 ran at 100Hz)
...
// time, each flown before the next.
#include <stdint.h>
#include <stdbool.h>
#include "app.h"
#include "FreeRTOS.h"
...
// --- Recording ------------------------------------------------------------
// 14 bytes, and the size is load-bearing. A download packet is 1 header +
// 2 index + sizeof(FlightSample) = 17 bytes, which fits ONE 20-byte BLE
// notification. Larger packets are split across two, and split packets arrive
// corrupted on this hardware -- the nRF51's s130 stack caps ATT_MTU at 23 and
// it cannot be raised. If you add a field, re-check that total.
typedef struct __attribute__((packed)) {
  int16_t  x, y, z;                   // position, mm
  uint16_t front, back, left, right;  // wall distances, mm (0 = out of range)
} FlightSample;

// 1Hz sampling, so 180 is three minutes of flight -- well past the one minute
// we fly. 180 * 14 = 2.5KB of static RAM.
#define MAX_SAMPLES 180
static FlightSample flight_log[MAX_SAMPLES];
static uint16_t sample_count = 0;
static uint16_t tele_samples = 0;   // published copy, so the app can see it
...
// One sample per packet. The index travels with it so the app can tell a
// dropped packet from a shifted one.
...
void appMain(void) {
  ...
  while (1) {
    ...
    // Download requests. Safe at any time: this only ever reads the buffer,
    // and the app is only connected when the drone is on the ground.
    if (clear_requested) {
    ...
    // One sample per second while airborne. Deliberately driven off is_flying
    // rather than a phase machine -- there is no phase machine here, and that
    // is the point.
    if (is_flying && (xTaskGetTickCount() - last_sample_tick) >= M2T(1000)) {
      ...
      if (sample_count < MAX_SAMPLES) {
        ...
        fs->front = tele_front;  fs->back  = tele_back;
        fs->left  = tele_left;   fs->right = tele_right;
        sample_count++;
    ...
    if (mission_guards && mission_state == 1 && !is_flying && !tele_canfly) {
        ...
        is_flying = true;
        // Each flight starts a fresh recording. The previous one is dropped
        // whether or not it was downloaded: unlike v2, the drone never parks
        // itself waiting for the app to acknowledge, because that left it
        // stuck refusing new missions when the app had no way to acknowledge.
        sample_count = 0;
        tele_samples = 0;
        last_sample_tick = xTaskGetTickCount();
        ...
        if ((int32_t)(xTaskGetTickCount() - hover_deadline) >= 0) {
            tele_endwhy = 1;
            DEBUG_PRINT("CAVEBAT: FLIGHT OK, peak %d of %d mm, %d samples recorded\n",
